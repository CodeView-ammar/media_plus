...
{
    public class DuplicationCheckUserNameEn: ValidationAttribute
    {
        private readonly UnitOfWork _unitOfWork = new();
        private readonly IRepository<User> _userTb;

        public DuplicationCheckUserNameEn()
        {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            && _userTb.EntitiesIQueryable()
                .Any(x=> x.UserNameEn == input && x.UserId != Convert.ToInt32(id) && x.UserCustCode == currentCustCode))
            {
                return new ValidationResult(GetErrorMessage(currentCulture));
            }
            
            if(_route.Contains("Create")
            &&_userTb.EntitiesIQueryable().Any(x=> x.UserNameEn == input && x.UserCustCode == currentCustCode)){
                return new ValidationResult(GetErrorMessage(currentCulture));
            }
        ...
        {
            var error = currentCulture == "en-US" ? "English User Name already exists." : "هذا الأسم بالأنجليزية موجود بالفعل الرجاء ادخال اسم اخر.";
            return error;
        }
    }
}