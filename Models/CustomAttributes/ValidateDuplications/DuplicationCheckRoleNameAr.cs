...
{
    public class DuplicationCheckRoleNameAr : ValidationAttribute
    {
        private readonly UnitOfWork _unitOfWork = new();
        private readonly IRepository<UserRole> _userRoleTb;

        public DuplicationCheckRoleNameAr()
        {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            && _userRoleTb.EntitiesIQueryable()
                .Any(x=> x.RoleNameAr == input && x.RoleId != Convert.ToInt32(id) && x.RoleCustCode == currentCustCode))
            {
                return new ValidationResult(GetErrorMessage(currentCulture));
            }
            if(_route.Contains("Create")
            &&_userRoleTb.EntitiesIQueryable().Any(x=> x.RoleNameAr == input && x.RoleCustCode == currentCustCode)){
                return new ValidationResult(GetErrorMessage(currentCulture));
            }
        ...
        {
             var error = currentCulture == "en-US" ? "Arabic Role Name already exists." : "هذا الأسم بالعربية موجود بالفعل الرجاء ادخال اسم اخر.";
            return error;
        }
    }
}