...
{
    public class DuplicationCheckUserNameAr: ValidationAttribute
    {
        private readonly UnitOfWork _unitOfWork = new();
        private readonly IRepository<User> _userTb;

        public DuplicationCheckUserNameAr()
        {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            && _userTb.EntitiesIQueryable()
                .Any(x=> x.UserNameAr == input && x.UserId != Convert.ToInt32(id) && x.UserCustCode == currentCustCode))
            {
            ...
            &&_userTb.EntitiesIQueryable().Any(x=> x.UserNameAr == input && x.UserCustCode == currentCustCode)){
                return new ValidationResult(GetErrorMessage(currentCulture));
            }
        ...
        {
            var error = currentCulture == "en-US" ? "Arabic User Name already exists." : "هذا الأسم بالعربية موجود بالفعل الرجاء ادخال اسم اخر.";
            return error;
        }
    }
}