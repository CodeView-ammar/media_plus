...
{
    public class DuplicationCheckMtypeNameEn : ValidationAttribute
    {
        private readonly UnitOfWork _unitOfWork = new();
        private readonly IRepository<MaterialType> _materialTypeTb;

        public DuplicationCheckMtypeNameEn()
        {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            && _materialTypeTb.EntitiesIQueryable()
                .Any(x=> x.MtypeNameEn == input && x.MtypeId != Convert.ToInt32(id) && x.MtypeCustCode == currentCustCode))
            {
                return new ValidationResult(GetErrorMessage());
            }
             if(_route.Contains("Create")
            &&_materialTypeTb.EntitiesIQueryable().Any(x=> x.MtypeNameEn == input  && x.MtypeCustCode == currentCustCode)){
                return new ValidationResult(GetErrorMessage());
            }
        ...
        {
            return "English Material Type Name already exists.";
        }
    }
}