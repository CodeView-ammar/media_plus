...
{
    public class DuplicationCheckCustomerEn: ValidationAttribute
    {
            private readonly UnitOfWork _unitOfWork = new();
        private readonly IRepository<Customer> _customerTb;

        public DuplicationCheckCustomerEn()
        {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            && _customerTb.EntitiesIQueryable()
                          .Any(x=> x.CustNameEn == customerName && x.CustId != Convert.ToInt32(id)))
            {
                return new ValidationResult(GetErrorMessage(currentCulture));
            }
            if(_route.Contains("Create")
            && _customerTb.EntitiesIQueryable().Any(x=> x.CustNameEn == customerName)){
                return new ValidationResult(GetErrorMessage(currentCulture));
            }
