...
{
    public class DuplicationCheckTempNameAr : ValidationAttribute
    {
        private readonly UnitOfWork _unitOfWork = new();
        private readonly IRepository<ShowTemplate> _showTemplateTb;

        public DuplicationCheckTempNameAr()
        {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            && _showTemplateTb.EntitiesIQueryable()
                .Any(x=> x.TempNameAr == input && x.TempId != Convert.ToInt32(id) && x.TempCustCode == currentCustCode))
            {
                return new ValidationResult(GetErrorMessage(currentCulture));
            }
            if(_route.Contains("Create")
            &&_showTemplateTb.EntitiesIQueryable().Any(x=> x.TempNameAr == input && x.TempCustCode == currentCustCode)){
                return new ValidationResult(GetErrorMessage(currentCulture));
            }
        ...
        {
            var error = currentCulture == "en-US" ? "Arabic Template Name already exists." : "هذا الأسم بالعربية موجود بالفعل الرجاء ادخال اسم اخر.";
            return error;
        }
    }
}